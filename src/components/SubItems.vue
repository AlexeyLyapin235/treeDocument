<template>
  <ul class="items">
    <li class="items__children-list">
      <div class="items__children-content">
        <span v-if="removeText == false" class="items__children-name">{{
          child.name
        }}</span>
        <input
          v-if="removeText == true"
          class="items__children-input"
          v-model="child.name"
        />
        <span class="items__children-Availability">{{
          child.Availability
        }}</span>
        <div class="items__buttons">
          <button
            ...
            :value="child.id"
            @click="this.$emit('delSubItem', child, itemsChid)"
          >
            <img
          ...
          <button
            ...
            @mouseover="this.sumMouse(true)"
            @mouseleave="this.sumMouse(false)"
          >
            <img
...
<style lang="scss">
.items__children-list {

  .items__children-content {
    width: 1150px;
    height: 35px;
    filter: drop-shadow(0px 4px 4px rgba(0, 0, 0, -0.75));
    border: 0.1px solid #bfc9e0;
    display: flex;
  
    .items__children-input {
      border: 0px;
      width: 300px;
    }
    .items__children-name {
      margin: 10px;
    }
    .items__children-Availability {
